...
const BackgroundRemovalToggle = ({
  ...
  // 캐시된 결과 저장
  const [cachedResults, setCachedResults] = useState<Map<string, string>>(new Map());
  ...
  const handleBackgroundRemoval = async () => {
    ...
    try {
      ...
      // 결과를 캐시에 저장
      setCachedResults(prev => new Map(prev).set(imageUrl, processedImageUrl));
      ...
      setError(error instanceof Error ? error.message : '배경 제거에 실패했습니다. 원본 이미지를 사용합니다.');
      onProcessedImageChange(null);
  ...
  const hasCachedResult = selectedAvatar?.avatarImage ? 
    cachedResults.has(selectedAvatar.avatarImage) : false;
    ...
    <div className="border-t pt-6 mt-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            ...
            {hasCachedResult && (
              <span className="text-green-600 font-medium"> (이미 처리된 이미지)</span>
            )}
          </p>
        </div>
      ...
      {isProcessing && (
        <div className="flex items-center space-x-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
          ...
          <div>
            <p className="text-sm font-medium text-purple-800">배경을 제거하고 있습니다...</p>
            <p className="text-xs text-purple-600">잠시만 기다려주세요 (약 3-10초 소요)</p>
          </div>
        </div>
      )}
      ...
      {isEnabled && !isProcessing && !error && (
        <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center space-x-2">
            ...
            <p className="text-sm text-green-700 font-medium">배경 제거가 완료되었습니다!</p>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
        ...
        <ul className="text-xs text-gray-600 space-y-1">
          ...
          <li>• 사람이 명확하게 보이는 아바타에서 가장 좋은 결과를 얻을 수 있습니다</li>
