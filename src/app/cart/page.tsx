...
const Cart = () => {
  ...
  const handleQuantityChange = (itemId: string, change: number) => {
    setCartItems(
      cartItems.map((item) => (item.id === itemId ? { ...item, quantity: Math.max(1, item.quantity + change) } : item)),
    )
  }
  ...
  const selectedCartItems = cartItems.filter((item) => selectedItems.includes(item.id))
  const totalProductPrice = selectedCartItems.reduce((sum, item) => sum + item.salePrice * item.quantity, 0)
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex items-center justify-between mb-6 pb-4 border-b">
            <div className="flex items-center space-x-2">
              ...
              <label className="text-sm font-medium cursor-pointer text-black">전체 선택</label>
            </div>
            <Button 
              variant="outline" 
              size="sm" 
          ...
          <div className="space-y-8">
            {Object.entries(groupedItems).map(([category, items]) => (
              <div key={category}>
                <h2 className="text-lg font-semibold mb-4 text-black">{category}</h2>
                      ...
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="font-medium text-sm mb-1 text-black">{item.name}</h3>
                              <p className="text-sm text-black mb-2">{item.size} / 1개</p>
                              <div className="flex items-center space-x-2">
                                <span className="text-sm text-black line-through">
                                  {item.originalPrice.toLocaleString()}원
                                </span>
                                <span className="font-bold text-lg text-black">{item.salePrice.toLocaleString()}원</span>
                              </div>
                            </div>
                          ...
                          <div className="mt-4 text-xs text-black mb-3">
                            택배 배송 07/1(화) 밤 12시 이전 주문 시 내일 도착 예정
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Button variant="outline" className="border-black text-black hover:bg-black hover:text-white" size="sm">
                              구매 서용
                            </Button>
                          </div>
                        </div>
          ...
          <div className="bg-gray-50 rounded-lg p-6 sticky top-4">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between items-center">
                <span className="text-lg font-bold text-black">총 구매 금액</span>
                <span className="text-xl font-bold text-black">{totalPrice.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between text-sm text-black">
                <span>적립예정 마일리지</span>
                <span>최대 0원</span>
